...
- **Storage** — Upstash Redis (KV REST), Vercel Blob (`EVENTS_BLOB_READ_WRITE_TOKEN` and `SHIPS_BLOB_READ_WRITE_TOKEN`, one token per store), Turso libSQL (privacy DB and ship DB).
