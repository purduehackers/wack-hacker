...
- **Storage** — Upstash Redis (KV REST), Vercel Blob (`SHIPS_BLOB_READ_WRITE_TOKEN`; plus `EVENTS_BLOB_READ_WRITE_TOKEN` pending teardown), Payload CMS for hack-night photos (`PAYLOAD_CMS_URL`, `PAYLOAD_SERVICE_ACCOUNT_API_KEY`), Turso libSQL (privacy DB and ship DB).
