...
| Tools         | All domain tools + `loadSkill`, then run through [`applyPolicy`](policy.md) — tools above the caller's role are omitted, confirm-gated tools get the Discord approval flow, and destructive runs are audited; Anthropic cache-control is layered at construction (`PrepareStepResult` has no `tools` key, so per-step application would be silently ignored) |
...
Each domain's entry in the **generated** `DOMAINS` registry (`src/lib/ai/skills/generated/domains.ts`, emitted by `compile-skills.ts` — don't edit it) provides:

- `tools: ToolSet` — every tool the subagent **could** call (the `src/lib/ai/tools/<domain>/index.ts` barrel).
- `subSkills: Record<string, SkillBundle>` — the per-domain sub-skill manifest.
- `baseToolNames: readonly string[]` — the discovery/search tools to activate on the first step, from the `baseTools` frontmatter in the domain's top-level `SKILL.md`.

At call time, `buildDelegationTools` (in `delegates.ts`) layers on:

- `name` — the domain key (e.g. `"linear"`).
- `description` — the top-level `SKILL.md`'s `description` plus its `criteria` ("…. Use when: …"), shown to the orchestrator on the delegation tool.
- `systemPrompt` — the same `SKILL.md` body, used as the subagent's instructions.

## Per-domain overrides (`DOMAIN_SPEC_OVERRIDES`)

Most subagents use the defaults. `delegates.ts` keeps a hand-written `DOMAIN_SPEC_OVERRIDES` map — a `Partial<Record<domain, Partial<SubagentSpec>>>` — for domains that need non-default wiring (these reference runtime functions a compiler can't emit). The overridable fields are:
