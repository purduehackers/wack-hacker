...
`feature-parity.json` is the reviewed capability-surface snapshot. It is generated from the
canonical sources, not from totals copied out of the migration plan:

- `skill-sources/*/SKILL.md` supplies base tools and sub-skill registrations.
- `agent/subagents/*/lib/tool-registry.ts` plus its `defineTool` exports supplies domain tools.
- `agent/subagents/*/agent.ts` supplies the declared subagent set.
- `lib/skills.generated.ts` is checked against the parsed skill sources after compilation.

`bun run check:parity` fails when those sources disagree with each other, a generated skill
module disagrees with its inputs, or the reviewed snapshot changes. For an intentional
capability change, inspect the diff and run `bun run parity:update`. The agents package runs
the check from `typecheck`, `test`, `build`, and `info`, so the repository CI exercises it.
...
| Surface                    | Legacy source inventory | Current generated surface | Reconciliation                                                                                                                                                                                                                                       |
| -------------------------- | ----------------------: | ------------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Sub-skills                 |   109 files, 108 active |         104 skill sources | Four active skills belonged to the specialized code domain; one `_outreach-send` skill was compiler-disabled. The remaining API-domain sources are `109 - 4 - 1 = 104`.                                                                              |
...
11 generated API domains plus the `code` and `docs` auxiliary subagents.
