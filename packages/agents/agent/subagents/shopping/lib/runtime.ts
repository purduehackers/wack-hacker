...
import { descriptorForTool, isShoppingToolName } from "./descriptors.ts";
...
async function evaluationContext(principal: PolicyPrincipal): Promise<PolicyEvaluationContext> {
  ...
  if (Result.isError(budget)) {
    ...
    console.warn("shopping budget lookup unavailable", serializeError(budget.error));
    return {};
  }
...
function upstreamFailure(cause: unknown, operation: string) {
  ...
  if (status === 429) return new RateLimited({ service: "Shopping", retryAfterMs: 1_000 });
...
function missingIntegration(name: ShoppingToolName): string | undefined {
  if (name === "search_products") {
    return env.SERPAPI_API_KEY === undefined ? "SERPAPI_API_KEY is not configured" : undefined;
  }
...
async function audit(
  ...
  const spec = SHOPPING_TOOLS[name];
  ...
  if (Result.isError(recorded)) {
    ...
    console.warn("shopping audit append unavailable", serializeError(recorded.error));
  }
}

export async function visibleShoppingToolNames(
  current: ApprovalContext["session"]["auth"]["current"],
  candidates: readonly string[],
): Promise<ShoppingToolName[]> {
  ...
  return candidates.filter((toolName): toolName is ShoppingToolName => {
    if (!isShoppingToolName(toolName)) return false;
...
export async function approvalForShoppingTool(
  name: ShoppingToolName,
  ctx: ApprovalContext,
    ...
    return {
      ...
      reason: Result.isError(decision)
        ...
        : decision.value.denial === "budget"
          ...
          : "Policy denied this Shopping action.",
    };
  }
...
async function principalForExecution(name: ShoppingToolName, ctx: ToolContext) {
...
export async function executeShoppingTool(
  ...
  const missing = missingIntegration(name);
  if (missing !== undefined) {
    const error = new UpstreamError({ service: "Shopping", status: 401, detail: missing });
  ...
  const spec = SHOPPING_TOOLS[name] as ShoppingToolSpec<z.ZodType>;
  ...
  const result = await Result.tryPromise({
    ...
    catch: (cause) => upstreamFailure(cause, `execute Shopping tool ${name}`),
  });
