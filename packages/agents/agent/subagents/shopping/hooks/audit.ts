...
import { descriptorForTool, isShoppingToolName } from "../lib/descriptors.ts";
  ...
  events: {
    async "actions.requested"(event, ctx) {
      ...
      for (const action of event.data.actions) {
        if (action.kind !== "tool-call" || !isShoppingToolName(action.toolName)) continue;
        ...
        if (Result.isError(recorded)) {
          console.warn("Shopping action audit hook unavailable", serializeError(recorded.error));
        }
      }
    },
  },
