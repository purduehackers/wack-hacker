 ...
 * are the same fact seen twice. Splitting them lets a tool exist that no
 * skill describes. `tool_defs/` mirrors the skill list exactly, and
 * `check:capabilities` fails if it stops doing so.
 *
 * Skill prose lives in `lib/skill_defs/<name>.md`, which this module imports as
 * text. The markdown remains a real document while policy stays here next to
 * the tools.
 */
