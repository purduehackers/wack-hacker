...
import { descriptorForTool, isNotionToolName } from "../lib/descriptors.ts";
  ...
  events: {
    async "actions.requested"(event, ctx) {
      ...
      for (const action of event.data.actions) {
        if (action.kind !== "tool-call" || !isNotionToolName(action.toolName)) continue;
        ...
        if (Result.isError(recorded)) {
          console.warn("Notion action audit hook unavailable", serializeError(recorded.error));
        }
      }
    },
  },
