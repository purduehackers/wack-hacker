...
import { descriptorForTool, isSentryToolName } from "../lib/descriptors.ts";
  ...
  events: {
    async "actions.requested"(event, ctx) {
      ...
      for (const action of event.data.actions) {
        if (action.kind !== "tool-call" || !isSentryToolName(action.toolName)) continue;
        ...
        if (Result.isError(recorded)) {
          console.warn("Sentry action audit hook unavailable", serializeError(recorded.error));
        }
      }
    },
  },
