...
import { descriptorForTool, isSentryToolName } from "./descriptors.ts";
...
async function evaluationContext(principal: PolicyPrincipal): Promise<PolicyEvaluationContext> {
  ...
  if (Result.isError(budget)) {
    ...
    console.warn("sentry budget lookup unavailable", serializeError(budget.error));
    return {};
  }
...
function upstreamFailure(cause: unknown, operation: string) {
  ...
  if (status === 429) return new RateLimited({ service: "Sentry", retryAfterMs: 1_000 });
...
async function audit(
  ...
  if (Result.isError(recorded)) {
    ...
    console.warn("sentry audit append unavailable", serializeError(recorded.error));
  }
}

export async function visibleSentryToolNames(
  current: ApprovalContext["session"]["auth"]["current"],
  requestedNames: readonly string[],
): Promise<SentryToolName[]> {
  ...
  return requestedNames.filter((toolName): toolName is SentryToolName => {
    if (!isSentryToolName(toolName)) return false;
...
export async function approvalForSentryTool(
  name: SentryToolName,
  ctx: ApprovalContext,
    ...
    return {
      ...
      reason: Result.isError(decision)
        ...
        : decision.value.denial === "budget"
          ...
          : "Policy denied this Sentry action.",
    };
  }
...
async function principalForExecution(name: SentryToolName, ctx: ToolContext) {
...
export async function executeSentryTool(
  ...
  if (env.SENTRY_AUTH_TOKEN === undefined || env.SENTRY_ORG === undefined) {
  ...
  const spec = SENTRY_TOOLS[name] as SentryToolSpec<z.ZodType>;
  ...
  const result = await Result.tryPromise({
    ...
    catch: (cause) => upstreamFailure(cause, `execute Sentry tool ${name}`),
  });
