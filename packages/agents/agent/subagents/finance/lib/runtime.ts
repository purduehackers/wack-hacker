...
import { descriptorForTool, isFinanceToolName } from "./descriptors.ts";
...
async function evaluationContext(principal: PolicyPrincipal): Promise<PolicyEvaluationContext> {
  ...
  if (Result.isError(budget)) {
    ...
    console.warn("finance budget lookup unavailable", serializeError(budget.error));
    return {};
  }
...
function upstreamFailure(cause: unknown, operation: string) {
  ...
  if (status === 429) return new RateLimited({ service: "Finance", retryAfterMs: 1_000 });
...
function missingIntegration(): string | undefined {
  return env.HCB_ORG_SLUG === undefined ? "HCB_ORG_SLUG is not configured" : undefined;
}
...
async function audit(
  ...
  if (Result.isError(recorded)) {
    ...
    console.warn("finance audit append unavailable", serializeError(recorded.error));
  }
}

export async function visibleFinanceToolNames(
  current: ApprovalContext["session"]["auth"]["current"],
  candidates: readonly string[],
): Promise<FinanceToolName[]> {
  ...
  return candidates.filter((toolName): toolName is FinanceToolName => {
    if (!isFinanceToolName(toolName)) return false;
...
export async function approvalForFinanceTool(
  name: FinanceToolName,
  ctx: ApprovalContext,
    ...
    return {
      ...
      reason: Result.isError(decision)
        ...
        : decision.value.denial === "budget"
          ...
          : "Policy denied this Finance action.",
    };
  }
...
async function principalForExecution(name: FinanceToolName, ctx: ToolContext) {
...
export async function executeFinanceTool(
  ...
  if (missing !== undefined) {
    const error = new UpstreamError({ service: "Hack Club Bank", status: 401, detail: missing });
  ...
  const spec = FINANCE_TOOLS[name] as FinanceToolSpec<z.ZodType>;
  ...
  const result = await Result.tryPromise({
    ...
    catch: (cause) => upstreamFailure(cause, `execute Finance tool ${name}`),
  });
