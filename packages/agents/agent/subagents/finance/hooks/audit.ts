...
import { descriptorForTool, isFinanceToolName } from "../lib/descriptors.ts";
  ...
  events: {
    async "actions.requested"(event, ctx) {
      ...
      for (const action of event.data.actions) {
        if (action.kind !== "tool-call" || !isFinanceToolName(action.toolName)) continue;
        ...
        if (Result.isError(recorded)) {
          console.warn("Finance action audit hook unavailable", serializeError(recorded.error));
        }
      }
    },
  },
