...
import {
  availableFinanceSkills,
  loadFinanceSkill,
  progressiveFinanceToolNames,
} from "../lib/skills.ts";
...
export default defineDynamic({
  events: {
    "turn.started": async (_event, ctx) => {
      const availableSkills = availableFinanceSkills(ctx.session.auth.current);
      const requestedNames = progressiveFinanceToolNames(ctx.messages);
      const visibleNames = await visibleFinanceToolNames(ctx.session.auth.current, requestedNames);
      const tools: Record<string, unknown> = {
        load_skill: defineTool({
          description:
            "Load a Finance skill before using its specialized tools. Available: " +
            availableSkills.map((skill) => `${skill.name} — ${skill.description}`).join("; "),
          inputSchema: z.object({ name: z.string().min(1) }),
          execute: async (input, toolCtx) => {
            return guardToolExecution(async () => {
              const loaded = loadFinanceSkill(input.name, toolCtx.session.auth.current);
              if (loaded.status === "error") {
                return { ok: false, error: serializeError(loaded.error) };
              }
              return {
                ok: true,
                activation: `finance.skill.loaded:${loaded.value.name}`,
