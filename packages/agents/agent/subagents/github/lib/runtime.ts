...
async function evaluationContext(principal: PolicyPrincipal): Promise<PolicyEvaluationContext> {
  ...
  if (Result.isError(budget)) {
    ...
    console.warn("github budget lookup unavailable", serializeError(budget.error));
    return {};
  }
...
function upstreamFailure(cause: unknown, operation: string) {
  ...
  if (status === 429) return new RateLimited({ service: "GitHub", retryAfterMs: 1_000 });
...
async function audit(
  ...
  if (Result.isError(recorded)) {
    ...
    console.warn("github audit append unavailable", serializeError(recorded.error));
  }
}

export async function visibleGithubToolNames(
  current: ApprovalContext["session"]["auth"]["current"],
  requestedNames: readonly string[],
): Promise<GithubToolName[]> {
  ...
  return requestedNames.filter((toolName): toolName is GithubToolName => {
    if (!isGithubToolName(toolName)) return false;
...
export async function approvalForGithubTool(
  name: GithubToolName,
  ctx: ApprovalContext,
    ...
    return {
      ...
      reason: Result.isError(decision)
        ...
        : decision.value.denial === "budget"
          ...
          : "Policy denied this GitHub action.",
    };
  }
...
async function principalForExecution(name: GithubToolName, ctx: ToolContext) {
...
export async function executeGithubTool(
  ...
  const result = await Result.tryPromise({
    ...
    catch: (cause) => upstreamFailure(cause, `execute GitHub tool ${name}`),
  });
