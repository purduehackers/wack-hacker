...
import { descriptorForTool, isGithubToolName } from "../lib/descriptors.ts";
  ...
  events: {
    async "actions.requested"(event, ctx) {
      ...
      for (const action of event.data.actions) {
        if (action.kind !== "tool-call" || !isGithubToolName(action.toolName)) continue;
        ...
        if (Result.isError(recorded)) {
          console.warn("GitHub action audit hook unavailable", serializeError(recorded.error));
        }
      }
    },
  },
