...
import { descriptorForTool, isOutreachToolName } from "../lib/descriptors.ts";
  ...
  events: {
    async "actions.requested"(event, ctx) {
      ...
      for (const action of event.data.actions) {
        if (action.kind !== "tool-call" || !isOutreachToolName(action.toolName)) continue;
        ...
        if (Result.isError(recorded)) {
          console.warn("Outreach action audit hook unavailable", serializeError(recorded.error));
        }
      }
    },
  },
