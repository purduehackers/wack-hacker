 ...
 * are the same fact seen twice. Splitting them is what let the grouped modules
 * accumulate tools no single skill described. `tool_defs/` mirrors the skill
 * list exactly, and `check:capabilities` fails if it stops doing so.
 *
 * Skill prose lives in `lib/skill_defs/<name>.md`, and this module imports it
 * as text. The markdown is then a real document while policy stays here next
 * to the tools.
 */
