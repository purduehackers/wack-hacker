...
async function evaluationContext(principal: PolicyPrincipal): Promise<PolicyEvaluationContext> {
  ...
  if (Result.isError(budget)) {
    ...
    console.warn("outreach budget lookup unavailable", serializeError(budget.error));
    return {};
  }
...
function upstreamFailure(cause: unknown, operation: string) {
  ...
  if (status === 429) return new RateLimited({ service: "Outreach", retryAfterMs: 1_000 });
...
function missingIntegration(name: OutreachToolName, input: unknown): string | undefined {
  ...
  if (HUNTER_TOOL_NAMES.has(name) && env.HUNTER_API_KEY === undefined) {
    return "HUNTER_API_KEY is not configured";
  }
  const usesNotion =
  ...
  if (usesNotion && env.NOTION_TOKEN === undefined) return "NOTION_TOKEN is not configured";
  return undefined;
}

async function audit(
  principal: PolicyPrincipal,
  name: OutreachToolName,
  input: unknown,
  decision: (typeof AuditDecision)[keyof typeof AuditDecision],
  decidedBy?: string,
): Promise<void> {
  const spec = OUTREACH_TOOLS[name];
  ...
  if (Result.isError(recorded)) {
    ...
    console.warn("outreach audit append unavailable", serializeError(recorded.error));
  }
}

export async function visibleOutreachToolNames(
  current: ApprovalContext["session"]["auth"]["current"],
  candidates: readonly string[],
): Promise<OutreachToolName[]> {
  ...
  return candidates.filter((toolName): toolName is OutreachToolName => {
    if (!isOutreachToolName(toolName)) return false;
...
export async function approvalForOutreachTool(
  name: OutreachToolName,
  ctx: ApprovalContext,
    ...
    return {
      ...
      reason: Result.isError(decision)
        ...
        : decision.value.denial === "budget"
          ...
          : "Policy denied this Outreach action.",
    };
  }
...
async function principalForExecution(name: OutreachToolName, ctx: ToolContext) {
...
export async function executeOutreachTool(
  ...
  const missing = missingIntegration(name, input);
  if (missing !== undefined) {
    const error = new UpstreamError({ service: "Outreach", status: 401, detail: missing });
  ...
  const result = await Result.tryPromise({
    ...
    catch: (cause) => upstreamFailure(cause, `execute Outreach tool ${name}`),
  });
