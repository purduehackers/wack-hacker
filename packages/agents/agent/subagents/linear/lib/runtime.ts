...
import { descriptorForTool, isLinearToolName } from "./descriptors.ts";
...
async function evaluationContext(principal: PolicyPrincipal): Promise<PolicyEvaluationContext> {
  ...
  if (Result.isError(budget)) {
    ...
    console.warn("linear budget lookup unavailable", serializeError(budget.error));
    return {};
  }
...
function upstreamFailure(cause: unknown, operation: string) {
  ...
  if (status === 429) return new RateLimited({ service: "Linear", retryAfterMs: 1_000 });
...
async function audit(
  ...
  if (Result.isError(recorded)) {
    ...
    console.warn("linear audit append unavailable", serializeError(recorded.error));
  }
}

export async function visibleLinearToolNames(
  current: ApprovalContext["session"]["auth"]["current"],
  candidates: readonly string[],
): Promise<LinearToolName[]> {
  ...
  return candidates.filter((toolName): toolName is LinearToolName => {
    if (!isLinearToolName(toolName)) return false;
...
export async function approvalForLinearTool(
  name: LinearToolName,
  ctx: ApprovalContext,
    ...
    return {
      ...
      reason: Result.isError(decision)
        ...
        : decision.value.denial === "budget"
          ...
          : "Policy denied this Linear action.",
    };
  }
...
async function principalForExecution(name: LinearToolName, ctx: ToolContext) {
...
export async function executeLinearTool(
  ...
  const spec = LINEAR_TOOLS[name] as LinearToolSpec<z.ZodType>;
  ...
  const result = await Result.tryPromise({
    ...
    catch: (cause) => upstreamFailure(cause, `execute Linear tool ${name}`),
  });
