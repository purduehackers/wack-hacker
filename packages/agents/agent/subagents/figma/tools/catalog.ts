...
import { availableFigmaSkills, loadFigmaSkill, progressiveFigmaToolNames } from "../lib/skills.ts";
import { FIGMA_TOOLS } from "../lib/tool-registry.ts";

export default defineDynamic({
  events: {
    "turn.started": async (_event, ctx) => {
      const availableSkills = availableFigmaSkills(ctx.session.auth.current);
      const requestedNames = progressiveFigmaToolNames(ctx.messages);
      const visibleNames = await visibleFigmaToolNames(ctx.session.auth.current, requestedNames);
      const tools: Record<string, unknown> = {
        load_skill: defineTool({
          description:
            "Load a Figma skill before using its specialized tools. Available: " +
            availableSkills.map((skill) => `${skill.name} — ${skill.description}`).join("; "),
          inputSchema: z.object({ name: z.string().min(1) }),
          execute: async (input, toolCtx) => {
            return guardToolExecution(async () => {
              const loaded = loadFigmaSkill(input.name, toolCtx.session.auth.current);
              if (loaded.status === "error") {
                return { ok: false, error: serializeError(loaded.error) };
              }
              return {
                ok: true,
                activation: `figma.skill.loaded:${loaded.value.name}`,
