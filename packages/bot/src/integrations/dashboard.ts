/**
 * Writes the hack night version to the dashboard's Vercel Global Config.
 *
 * The dashboard reads `version` from Global Config at request time, so this is
 * how `/hack-night start` changes what the site shows.
 *
...
export interface DashboardDeps {
  ...
  /** The dashboard's Global Config connection string, not the bot's own. */
  ...
  if (!connection) {
    return Result.err(
      new UpstreamError({
        ...
        status: 0,
        detail: "not a valid Global Config connection string",
      }),
    );
  }
...
export function createDashboardWriter(deps: DashboardDeps): Result<DashboardWriter, UpstreamError> {
  ...
  const url = `${VERCEL_API}/v1/global-config/${globalConfigId.value}/items`;
          ...
          try: async () => {
            ...
            if (!response.ok) {
              ...
              throw response.status < 500
                ...
                : new Transient({ operation: "global config upsert", detail });
            }
