 ...
 *
 * `allows*` is deliberately the whole export surface besides the machines: this
...
/** Where a legal delivery transition lands, or nothing when the machine refuses the event. */
...
/** Where a legal render transition lands, or nothing when the machine refuses the event. */
 ...
 * so anything other than a known phase name means a table grew a shape this
 * was not written for. That case reads as "no transition" rather than a guess,
 * so the lockstep check fails loudly instead of agreeing by accident.
 */
