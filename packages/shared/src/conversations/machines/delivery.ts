 ...
 * with the compare-and-set that makes it atomic. It stays there because moving
 * a guard out here would turn check-then-write into a race across two
 * processes.
 *
 ...
 * drives it in lockstep with those scripts, so a guard that drifts fails the
 * gate rather than surfacing later as a wedged conversation.
 */

import { setup } from "xstate";

/**
 * `expired` and `done` are both terminal and both delete the record. They stay
 * apart because only one is a normal ending, and the difference is what the
 * thread gets told.
 */
 ...
 *
 * Only the fields a *transition* depends on. Fencing identity and the leases
 * live on the record in Redis. Lua checks them there, in the same atomic step
 * as the write. Restating them here would imply this machine could enforce
 * them.
 */
export interface DeliveryContext {
  /** Empty until eve acknowledges the session. Several guards turn on this. */
  readonly sessionId: string;
  /** Whether the terminal paint is durable. The machine refuses `complete` without it. */
  readonly renderSettled: boolean;
}

/**
 * One shape rather than a discriminated union: no transition here carries a
 * payload, so a union buys nothing and costs inference. An annotated sample of
 * a union narrows to its initializer at xstate's `types` boundary. This
 * codebase bans the assertion that would fix that.
 */
...
export const deliveryMachine = setup({
  ...
  guards: {
    /** Nothing may release a turn before its final paint is durable. */
