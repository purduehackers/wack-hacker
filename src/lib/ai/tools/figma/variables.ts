...
export const get_local_variables = access(
  { risk: "read" },
  tool({
    description:
...
export const get_published_variables = access(
  { risk: "read" },
  tool({
    description:
...
export const modify_variables = access(
  { risk: "destructive" },
  tool({
    description:
    ...
    execute: async ({ file_key, variable_collections, variable_modes, variables }) => {
      ...
      const result = await figma.post<PostVariablesResponse>(
