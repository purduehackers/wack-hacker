...
export const list_broadcasts = access(
  { risk: "read" },
  tool({
    description:
...
export const get_broadcast = access(
  { risk: "read" },
  tool({
...
export const create_broadcast = access(
  { risk: "write" },
  tool({
    description:
    ...
    inputSchema: z.object({
      ...
      text: z
        .string()
        .describe("Plain-text body (required — used as fallback for non-HTML clients)"),
...
export const send_broadcast = access(
...
export const delete_broadcast = access(
  { risk: "destructive" },
  tool({
