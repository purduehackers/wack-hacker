...
function buildPrBody(lastText: string, authorUsername: string | undefined): string {
  const withoutCommitLine = lastText.replace(/^\s*\*\*Commit message\*\*\s*:.*$/im, "").trim();
  const attribution = authorUsername
    ? `\`Generated by @${authorUsername} using [Wack Hacker](https://github.com/purduehackers/wack-hacker)\``
