...
export const handleAutoThread = Effect.fn("AutoThread.handle")(
  function* (message: Message) {
    ...
    if (
      !(AUTO_THREAD_CHANNELS as readonly string[]).includes(message.channelId)
    ) {
    ...
    if (message.reference) {
      ...
      if (
        ref &&
        !ref.channel.isDMBased() &&
        ref.channel.parentId === SIGHORSE_CATEGORY_ID
      ) {
    ...
    if (!hasProjectLink && !hasAttachment && !isSIGHORSECheckpoint) {
      ...
      yield* Effect.logInfo(
        "message violates auto thread requirements deleting",
        {
          channel_id: message.channelId,
          message_id: message.id,
          user_id: message.author.id,
      ...
      yield* Effect.tryPromise({
        try: () => message.author.send(reminderMessage),
          ...
          new Error(
            `Failed to DM: ${e instanceof Error ? e.message : String(e)}`,
          ),
      }).pipe(
        Effect.tap(() =>
        ...
        Effect.catchAll((error) =>
          Effect.logWarning(
            "reminder dm send failed user may have dms disabled",
            {
              channel_id: message.channelId,
              message_id: message.id,
              user_id: message.author.id,
    ...
    const thread = yield* Effect.tryPromise({
      try: () =>
        ...
        new Error(
          `Failed to start thread: ${e instanceof Error ? e.message : String(e)}`,
        ),
    }).pipe(
      Effect.tap((thread) =>
        Effect.gen(function* () {
    ...
    if (message.channelId === CHECKPOINTS_CHANNEL_ID && hasWackyRole) {
      ...
      yield* Effect.tryPromise({
        ...
        Effect.tap(() =>
          Effect.logInfo(
            "checkpoint reactions and response added successfully",
            {
              channel_id: message.channelId,
              message_id: message.id,
              user_id: message.author.id,
