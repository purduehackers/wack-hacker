  ...
  function* (message: Message) {
    ...
    yield* Effect.tryPromise({
      try: () => message.react("\u2705"),
        ...
        new Error(
          `Failed to react: ${e instanceof Error ? e.message : String(e)}`,
        ),
    }).pipe(
      Effect.catchAll((e) =>
    ...
    effect.pipe(
      Effect.catchAll((e) =>
        Effect.logError("hack night image upload failed", {
          ...
          user_id: message.author.id,
          attachment_count: message.attachments.size,
          thread_name: message.channel.isThread()
...
export const handleHackNightImageRemoval = Effect.fn(
  "HackNight.handleImageRemoval",
)(
  function* (
    ...
    const removedCount = yield* storage.removeImagesForMessage(
      eventSlug,
      message.id,
    );
...
export const cleanupHackNightThread = Effect.fn("HackNight.cleanupThread")(
  function* (client: Client) {
    ...
    yield* Effect.logDebug("event index fetched", {
      ...
      image_count: Option.isSome(eventIndexOpt)
        ? eventIndexOpt.value.images.length
        : 0,
      duration_ms: Duration.toMillis(indexFetchDuration),
    });

    if (
      Option.isNone(eventIndexOpt) ||
      eventIndexOpt.value.images.length === 0
    ) {
      ...
      Effect.tryPromise({
        try: async () => {
          const starterMessage =
            await hackNightImageThread.fetchStarterMessage();
          ...
          if (winner) {
            const winnerMember = await textChannel.guild.members.fetch(
              winner[0],
            );
...
export const handleInitHnCommand = Effect.fn("HackNight.handleInitHn")(
  function* (interaction: ChatInputCommandInteraction) {
    ...
    yield* updateEdgeConfig(version);
