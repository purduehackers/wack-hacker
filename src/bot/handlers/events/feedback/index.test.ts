...
vi.mock("@/lib/logging/wide", () => ({
  createWideLogger: vi.fn((context: Record<string, unknown>) => {
    loggerContexts.push(context);
    return { set: vi.fn(), info: vi.fn(), warn: vi.fn(), error: vi.fn(), emit: emitted };
  }),
}));
vi.mock("@/lib/metrics", () => ({ countMetric: vi.fn() }));
...
describe("feedback reaction handler", () => {
  ...
  it.each([
    ...
    expect(countMetric).toHaveBeenCalledWith("ai.feedback", { emoji, positive });
  });
